...
# # Root directory where datasets should be used (and downloaded if not found)
# _C.DATASETS.ROOT_DIR = '/data/chenzy/datasets/duke_mtmc/DukeMTMC-VideoReID'
# Length of each training or testing Image Sequence
_C.DATASETS.SEQ_LEN = 4
# Squence sample methods, random means select a sized 1* SEQ_LEN image sequence from a video,
# dense means select a sized min(TEST_MAX_SEQ_NUM, len(video / SEQ_LEN)) * SEQ_LEN image sequences from a video
_C.DATASETS.TRAIN_SAMPLE_METHOD = "random"
_C.DATASETS.TEST_SAMPLE_METHOD = "dense"
# Max Seq Num When Testing
_C.DATASETS.TEST_MAX_SEQ_NUM = 200

# Datasets attributes settings
